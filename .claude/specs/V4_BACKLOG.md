...
| T-10 | Pre-warming continuo de funciones críticas | Causa raíz de los fixme: cold start + streaming SSR + Prisma sin `connection_limit=1` (no latencia de red — `regions: ["gru1"]` ya aplicaba a preview, PgBouncer ya activo). Solución V3: `connection_limit=1` en Prisma, `functionMaxDuration: 60`, test timeout 60s, warm-up en E2E workflow, loading.tsx per-page en 4 rutas críticas. Si en V4 hay rutas nuevas pesadas, recordar agregarles loading.tsx | 3h |
