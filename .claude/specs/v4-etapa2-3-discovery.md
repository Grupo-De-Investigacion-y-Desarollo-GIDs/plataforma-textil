...
> **Estado:** ✅ **COMPLETO — ETAPA 2 CERRADA.** **A (vidriera mínima) = HECHO** (PR #448, `9062603`). **B0 (schema + estado + helper + enforcement + banners) = HECHO** (PR #449, squash `23d20b0`, 2026-07-02). **B1 (cron + emails + reactivación + email de lanzamiento) = HECHO** (PR #450, squash `5b567d5` en develop, 2026-07-07, CI verde unit+e2e). Con B1 se cierra **2.3** y la **Etapa 2 completa** (2.1, 2.2, 2.3, 2.4, 2.5).
>
> **⚠️ Acuerdo operativo (Gerardo + Sergio):** el **email de lanzamiento** (`POST /api/cron/gracia-lanzamiento?confirmar=SI`) NO es automático. Se dispara **MANUALMENTE al salir a producción, coordinado con Sergio**, avisando a **UNTREF** y al **piloto** en paralelo. Queda registrado el acuerdo. Runbook: `.claude/specs/v4-etapa2-3b1-runbook.md`. Pendiente puntual de infra: `CRON_SECRET` seteado en Vercel (prod + preview) — sin él el endpoint responde 401.
...
- **B — Gracia 60 días:** **B0 ✅ HECHO (`23d20b0`)** = schema (enum `EstadoCuenta` + campos Taller + migración/backfill amnistía) + helper puro `clasificarGracia` + estado al alta + enforcement (verificado: todo lo comercial ya gatea en `verificadoAfip`) + banners countdown/reactivación. **B1 ✅ HECHO (`5b567d5`)** = cron (`api/cron/gracia-cuit`, `0 11 * * *`) + `CRON_SECRET` + `crons` en `vercel.json` + email recordatorio día 50 (`buildRecordatorioCuitEmail`, estado `VENCE_PRONTO`) + reactivación automática al verificar (`datosReactivacion` en `sincronizarTaller`) + email de lanzamiento one-off (`buildLanzamientoGraciaEmail` / `api/cron/gracia-lanzamiento`) + transición EN_GRACIA→INACTIVA por tiempo. Decisión pura `planificarAccionGracia`; tests unit (gracia + cron 401/idempotencia) + reactivación.
...
- **PR B1 — Cron + email + reactivación + tests ✅ HECHO (#450, squash `5b567d5`, mergeado 2026-07-07, CI verde unit+e2e):** endpoint `GET api/cron/gracia-cuit` (nodejs+force-dynamic, `Authorization: Bearer ${CRON_SECRET}`→401 sin secret) + `crons` en `vercel.json` (`0 11 * * *` = 08:00 ART) + `buildRecordatorioCuitEmail` (día 50, estado `VENCE_PRONTO`) + transición `EN_GRACIA→INACTIVA` por tiempo (`estadoCuenta`/`inactivadaAt`) + reactivación automática CENTRALIZADA en `sincronizarTaller` (`datosReactivacion()` → `estadoCuenta=ACTIVA`+`inicioGracia`/`inactivadaAt`/`recordatorioCuitEnviadoAt`=null; un solo punto cubre reverificación ESTADO + cualquier caller; registro/completar ya usan `estadoCuentaInicial`) + email de lanzamiento one-off (`buildLanzamientoGraciaEmail`, `POST api/cron/gracia-lanzamiento?confirmar=SI`, manual) + emails firma "Coordinación". Decisión pura `planificarAccionGracia` (NADA/RECORDATORIO/INACTIVAR, testeada con fechas fabricadas); route solo ejecuta, cada taller aislado en try/catch. Tests: `gracia.test.ts` (planificarAccionGracia + datosReactivacion) + `cron-gracia.test.ts` (401×3 + acciones + idempotencia doble-corrida) + `arca.test.ts` (reactivación). Runbook `v4-etapa2-3b1-runbook.md`. **Cierra 2.3 y la Etapa 2.** **Pendiente infra:** `CRON_SECRET` en Vercel. **Acuerdo:** email de lanzamiento se dispara manual al salir a prod, coordinado con Sergio (avisar UNTREF + piloto en paralelo).

Orden: ~~**A** (independiente)~~ ✅ · ~~**B0**~~ ✅ · ~~**B1**~~ ✅. **Etapa 2.3 COMPLETA → ETAPA 2 COMPLETA.**
